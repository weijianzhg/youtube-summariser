...
### Added
- **Short command aliases**: `yt-summarise` and `yt-summarize` as shorter alternatives to the full command names
- **Unquoted URL support**: YouTube URLs no longer need to be wrapped in quotes
  - `yt-summarise https://www.youtube.com/watch?v=ID` now works directly
  - Shell-split `&` fragments in URLs are automatically reassembled
